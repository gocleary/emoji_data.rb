...
module EmojiData
  ...
  EMOJI_CHARS.each do |ec|
    ...
    ec.skin_variations.each { |_key, value| EMOJICHAR_UNIFIED_MAP[value['unified']] = ec }
  end
  ...
  # Returns a list of all `EmojiChar` that are represented with multibyte
  # encoding.
  #
  # @return [Array<EmojiChar>] a list of all multibyte `EmojiChar`.
  def self.all_multibyte
    EMOJI_CHARS.select(&:multibyte?)
  end

  # Returns a list of all `EmojiChar` that have at least one skin variation.
  #
  # @return [Array<EmojiChar>] a list of all `EmojiChar` with skin variations.
  def self.all_with_variations
    EMOJI_CHARS.select(&:variations?)
  end
  ...
  # However, if you pass `include_variants: true` then all
  ...
  #
  # @include_variants [Boolean] whether or not to include all
  #   possible encoding variants in the list
  #
  # @return [Array<String>] all Emoji characters rendered as UTF-8 strings
  def self.chars(include_variants: false)
    if include_variants
      EMOJI_CHARS.map(&:chars).flatten
    else
      EMOJI_CHARS.map(&:render)
    end
  end
  ...
  def self.char_to_unified(char)
    EmojiChar.char_to_unified(char)
  end
  ...
  def self.find_by_name(name)
    find_by_value(:name, name.upcase)
  end
  ...
  def self.find_by_short_name(short_name)
    find_by_value(:short_name, short_name.downcase)
  end
  ...
  def self.find_by_value(field, value)
    all.select { |char| char.send(field).include? value }
  end
end
